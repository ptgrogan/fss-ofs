...
requirejs(['underscore','winston','child_process','minimist','mongojs'], function(_,logger,child_process,minimist,mongo) {
		...
		_.each([1,2,3,4,5,6], function(s) {
			numSGL = Math.max(numSGL, _.reduce(sats, function(memo, sat) {
				return (parseInt(sat.match(/EO(\d)/)[1])===s 
						&& (parseInt(sat.match(/(\d+)\./)[1])===(player+1)
							|| sgl.match(/oSGL/)))? 
	...
	function enumSymmetricPxNSats(number, players, sectors, sgl, isl) {
		...
		return _.map(_.filter(_.range(0,sats[0].length,1), function(i) {
					// require at least one satellite to have a sensor
					return _.some(sats[0][i], function(sat) {
						return sat.match(/SAR|VIS/g);
					});
				}), function(i) {
	...
	function enumSymmetricPxNSatDesigns(number, players, sectors, sgl, isl) {
		return _.map(enumSymmetricPxNSats(number, players, _.map(sectors, 
			...
			function(pSats) {
				return _.map(pSats, function(sats, i) {
					return [sats.join(" "), sizeStation(players[i],sectors[i],sgl,_.flatten(pSats))].join(" ");
				}).join(" ");
			});
	}
	
	var runs = _.union(
			enumSymmetricPxNSatDesigns(1,[1,2],[1,6],"pSGL","pISL"),
			enumSymmetricPxNSatDesigns(2,[1,2],[1,5],"pSGL","pISL"),
			enumSymmetricPxNSatDesigns(3,[1,2],[1,4],"pSGL","pISL"),
	...
	var runs = _.union(
			enumSymmetricPxNSatDesigns(4,[1,2],[1,4],"pSGL","pISL"),
			enumSymmetricPxNSatDesigns(4,[1,2],[1,4],"pSGL","oISL"),
			enumSymmetricPxNSatDesigns(4,[1,2],[1,4],"oSGL","pISL"),
			enumSymmetricPxNSatDesigns(4,[1,2],[1,4],"oSGL","oISL")
		);
	...
	function executeBatch(runs, seed, batchDone) {
		...
		_.each(runs, function(run) {
			...
			var ops = ' ';
			if(numPlayers===1 || (!run.match(/oSGL/) && !run.match(/oISL/g))) {
				ops = ' -o d6 ';
			} else if(numPlayers>1) {
				ops = ' -o n -f d6 ';
			}
			db.collection('exp6s').find({run:run,seed:seed}, function(err, docs) {
				if(err) {
					logger.error(err);
				} else if(docs.length>0) {
					db.collection('exp7s').insert(docs, function() {
						execCounter++;
