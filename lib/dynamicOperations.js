...
define('dynamicOperations', function(require) {
    ...
    DynamicOperations.prototype.operate = function(federate, context) {
        ...
        var contracts = _.clone(federate.contracts);
        
        var SGLprotocols = ['pSGL','oSGL'];
        var ISLprotocols = ['pISL','oISL'];
        var phenomenas = ['VIS','SAR']
        ...
        */
        ...
        var transportContract = function(spacecraft, contract, context) {
            var i = _.indexOf(spacecrafts, spacecraft);
            var j = _.indexOf(contracts, contract);
            if((data = federate.getData(contract)) !== undefined) {
                _.each(stations, function(station, k) {
                    _.each(SGLprotocols, function(protocol, l) {
                        if(lp.get(T_c[0][i][k][l][j]) && federate.canTransport(
                                data, protocol, spacecraft, station, context)) {
                            federate.transport(data, protocol, 
                ...
                _.each(spacecrafts, function(spacecraft2, k) {
                    _.each(ISLprotocols, function(protocol, l) {
                        if(lp.get(L_c[0][i][k][l][j]) && federate.canTransport(
                                data, protocol, spacecraft, spacecraft2, context)) {
                            federate.transport(data, protocol, 
                                    spacecraft, spacecraft2, context);
                            transportContract(spacecraft2, contract, context);
                        }
                    }, this);
                }, this);
            }
        ...
        var transportDemand = function(spacecraft, demand, context) {
            var i = _.indexOf(spacecrafts, spacecraft);
            var j = _.indexOf(demands, demand);
            if((contract = federate.getContract(demand)) !== undefined
                    && (data = federate.getData(contract)) !== undefined) {
                _.each(stations, function(station, k) {
                    _.each(SGLprotocols, function(protocol, l) {
                        if(lp.get(T_d[0][i][k][l][j]) && federate.canTransport(
                                data, protocol, spacecraft, station, context)) {
                            federate.transport(data, protocol, 
                                    spacecraft, station, context);
                            federate.resolveContract(contract, context);
                ...
                _.each(spacecrafts, function(spacecraft2, k) {
                    _.each(ISLprotocols, function(protocol, l) {
                        if(lp.get(L_d[0][i][k][l][j]) && federate.canTransport(
                                data, protocol, spacecraft, spacecraft2, context)) {
                            federate.transport(data, protocol, 
                                    spacecraft, spacecraft2, context);
                            transportDemand(spacecraft2, demand, context);
                        }
                    }, this);
                }, this);
            }
        ...
        _.each(systems, function(system, i) {
            if(system.isSpace()) {
                _.each(contracts, function(contract, j) {
                    if(lp.get(R_c[0][i][j])) {
                        console.log('purposefully defaulting contract');
        ...
        _.each(spacecrafts, function(spacecraft, i) {
            _.each(contracts, function(contract, j) {
                transportContract(spacecraft, contract, context);
            }, this);
        }, this);
        ...
        _.each(spacecrafts, function(spacecraft, i) {
            _.each(demands, function(demand, j) {
                transportDemand(spacecraft, demand, context);
            }, this);
        }, this);
    };
