...
define('dynamicOperations', function(require) {
    ...
    DynamicOperations.prototype.operate = function(federate, context) {
        ...
        var C = [];
        var E_0 = []; // E_0[i][j]: spacecraft i initially holds data for contract j
        var E = []; // E[t][i][j]: at time t spacecraft i holds data for contract j
        var T = []; // T[t][i][j][k][l]: at time t transmit data from spacecraft i to ground station j using protocol k for contract l
        var R = []; // R[t][i][j]: at time t resolve data in system i for contract j
        ...
        _.each(spacecrafts, function(spacecraft, i) {
            ...
            _.each(federate.contracts, function(contract, j) {
                E_0[i][j+_.size(demands)] = _.reduce(spacecraft.subsystems, function(memo, subsystem) {
        ...
        _.each(_.range(minTime, maxTime, 1), function(time, t) {
            ...
            _.each(spacecrafts, function(spacecraft, i) {
                ...
                _.each(demands, function(demand, j) {
                    ...
                    E[t][i][j] = lp.addColumn(spacecraft.id + "-E-" + demand.id+"@"+time, true);
                }, this);
                _.each(federate.contracts, function(contract, j) {
                    // spacecraft i holds data for contract j
                    E[t][i][j+_.size(demands)] = lp.addColumn(spacecraft.id + "-E-" + contract.demand.id+"@"+time, true);
                }, this);
                _.each(['VIS', 'SAR'], function(phenomena) {
                    var c = new lpsolve.Row();
                    _.each(demands, function(demand, j) {
                        if(demand.phenomena===phenomena) {
                            c.Add(E[t][i][j], demand.size);
                        }
                    }, this);
                    _.each(federate.contracts, function(contract, j) {
                        if(contract.demand.phenomena===phenomena) {
                            c.Add(E[t][i][j+_.size(demands)], contract.demand.size);
                        }
                    }, this);
                ...
                _.each(stations, function(station, j) {
                    ...
                    _.each(['pSGL', 'oSGL'], function(protocol, k) {
                        ...
                        _.each(demands, function(demand, l) {
                            ...
                            T[t][i][j][k][l] = lp.addColumn(spacecraft.id + "-" + station.id + '-T' + '(' + protocol + ')-' + demand.id+"@"+time, true);
                            c_tx.Add(T[t][i][j][k][l], demand.size);
                        
                            // constrain transmission by visibility
                            C.push({
                                row: new lpsolve.Row().Add(T[t][i][j][k][l], 1), 
                                constraint: 'LE', 
                        ...
                        _.each(federate.contracts, function(contract, l) {
                            ...
                            T[t][i][j][k][l+_.size(demands)] = lp.addColumn(spacecraft.id + '-' + station.id + '-T' + '(' + protocol + ')-' + contract.demand.id+"@"+time, true);
                            c_tx.Add(T[t][i][j][k][l+_.size(demands)], contract.demand.size);
                        
                            // constrain transmission by visibility
                            C.push({
                                row: new lpsolve.Row().Add(T[t][i][j][k][l+_.size(demands)], 1), 
                                constraint: 'LE', 
                                constant: (spacecraft.couldTransmit(protocol, 
            ...
            _.each(stations, function(station, j) {
                _.each(['pSGL', 'oSGL'], function(protocol, k) {
                    ...
                    _.each(spacecrafts, function(spacecraft, i) {
                        _.each(demands, function(demand, l) {
                            c_rx.Add(T[t][i][j][k][l], demand.size);
                        }, this);
                        _.each(federate.contracts, function(contract, l) {
                            c_rx.Add(T[t][i][j][k][l+_.size(demands)], contract.demand.size);
                        }, this);
                    }, this);
            ...
            _.each(systems, function(system, i) {
                ...
                _.each(demands, function(demand, j) {
                    ...
                    R[t][i][j] = lp.addColumn(system.id + "-R-" + demand.id+"@"+time);
                    if(system.isGround()) {
                        J.Add(R[t][i][j], demand.getValueAt(time-context.time));
                    } else {
                        J.Add(R[t][i][j], demand.defaultValue);
                    }
                }, this);
                _.each(federate.contracts, function(contract, j) {
                    // system i resolves data for contract j
                    R[t][i][j+_.size(demands)] = lp.addColumn(system.id + "-R-" + contract.demand.id+"@"+time);
                    if(system.isGround()) {
                        J.Add(R[t][i][j+_.size(demands)],contract.demand.getValueAt(
                                contract.elapsedTime + time-context.time));
                    } else {
                        J.Add(R[t][i][j+_.size(demands)], 
                                contract.demand.defaultValue);
                    }
                }, this);
            }, this);
            ...
            _.each(spacecrafts, function(spacecraft, i) {
                ...
                _.each(demands, function(demand, j) {
                    ...
                    if(time===minTime) {
                        ...
                        c_nf.Add(E[t-1][i][j],1);
                    }
                    c_nf.Add(E[t][i][j],-1).Add(R[t][R_i][j],-1);
                    _.each(stations, function(station, k) {
                       _.each(['pSGL','oSGL'], function(protocol, l) {
                            c_nf.Add(T[t][i][k][l][j],-1);
                       }, this);
                    }, this);
                ...
                _.each(federate.contracts, function(contract, j) {
                    ...
                    if(time===minTime) {
                        ...
                        c_nf.Add(E[t-1][i][j+_.size(demands)],1);
                    }
                    if(time===maxTime) {
                    }
                    c_nf.Add(E[t][i][j+_.size(demands)],-1);
                    c_nf.Add(R[t][R_i][j+_.size(demands)],-1);
                    _.each(stations, function(station, k) {
                       _.each(['pSGL','oSGL'], function(protocol, l) {
                            c_nf.Add(T[t][i][k][l][j+_.size(demands)],-1);
                       }, this);
                    }, this);
                    // constrain net flow at each spacecraft
                    C.push({
                        row: c_nf, 
                        constraint: 'EQ', 
                        constant: -1*(time===minTime?E_0[i][j+_.size(demands)]:0), 
                        name: spacecraft.id + ' net flow of ' + contract.demand.id + ' at ' + time
                    });
                    if(time===maxTime) {
                        var c_bf = new lpsolve.Row();
                        c_bf.Add(E[t][i][j+_.size(demands)],1);
                        // constrain boundary flow at each spacecraft
                        C.push({
                            row: c_bf, 
                            constraint: 'EQ', 
            ...
            _.each(stations, function(station, k) {
                ...
                _.each(federate.contracts, function(contract, j) {
                    var c_nf = new lpsolve.Row();
                    c_nf.Add(R[t][R_k][j+_.size(demands)],-1);
                    _.each(spacecrafts, function(spacecraft, i) {
                       _.each(['pSGL','oSGL'], function(protocol, l) {
                            c_nf.Add(T[t][i][k][l][j+_.size(demands)],1);
                       }, this);
        ...
        _.each(spacecrafts, function(spacecraft, i) {
            _.each(federate.contracts, function(contract, j) {
                _.each(stations, function(station, k) {
                    _.each(['pSGL','oSGL'], function(protocol, l) {
                        if(lp.get(T[0][i][k][l][j+_.size(demands)])) {
