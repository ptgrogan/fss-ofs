                ...
                _.each(spacecrafts, function(spacecraft2, j) {
                    ...
                    _.each(ISLprotocols, function(protocol, k) {
                        ...
                        _.each(demands, function(demand, l) {
							...
							// small penalty to discourage cycles and micromanagement (e.g. cannot exchange location of data)
                            J.Add(L_d[t][i][j][k][l], -25);
		...
		if(result.code>1) {
			...
			var transportContract = function(operations, spacecraft, contract, context) {
				...
				if((data = federate1.getData(contract)) !== undefined) {
					if(lp.get(R_c[0][R_i][j])) {
						// TODO does not seem to catch default
			...
			// zeroth, resolve any demands on orbit
			_.each(systems, function(system, i) {
				if(system.isSpace()) {
					_.each(contracts, function(contract, j) {
						if(lp.get(R_c[0][i][j])) {
							this.getContractOwner(contract, controller).defaultContract(contract, context);
						}
					}, this);
				}
			}, this);
			...
			// second, transport contracts to storage
			_.each(contracts, function(contract, j) {
				if(_.every(R_c[0], function(o,i) {
			...
			// third, sense and transport demands to resolution
			_.each(demands, function(demand, j) {
				if(_.some(R_d[0], function(o,i) {
			...
			// finally, sense and transport demands to storage
			_.each(demands, function(demand, j) {
				if(_.every(R_d[0], function(o,i) {
