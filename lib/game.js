...
define('game',function(require) {
    ...
    Game.prototype.buildContext = function(seed, ops, fops) {
		...
		if(fops !== undefined && fops.match(/d+/)) {
			...
			if(fops.match(/(\d+,\d+)/)) {
				downlinkCost = parseInt(fops.match(/(\d+,\d+)/)[1].split(',')[0]);
				crosslinkCost = parseInt(fops.match(/(\d+,\d+)/)[1].split(',')[1]);
			}
			...
			var crosslinkCost = 20;
			if(fops.match(/(\d+,\d+,\d+)/)) {
				downlinkCost = parseInt(fops.match(/(\d+,\d+,\d+)/)[1].split(',')[0]);
				crosslinkCost = parseInt(fops.match(/(\d+,\d+,\d+)/)[1].split(',')[1]);
				horizon = parseInt(fops.match(/(\d+,\d+,\d+)/)[1].split(',')[2]);
			}
