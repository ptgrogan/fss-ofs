    ...
    Game.prototype.buildContext = function(seed, ops, fops) {
        ...
        for(var i = 0; i < this.numPlayers; i++) {
            ...
            if(ops !== undefined && ops.match(/s/)) {
				...
				var islPenalty = -10;
				if(ops.match(/(\d+,(?:a|\d+),\d+)/)) {
					var m = ops.match(/(\d+,(?:a|\d+),\d+)/)[1].split(',');
					planningHorizon = parseInt(m[0]);
					storagePenalty = -1*parseInt(m[1]);
					islPenalty = -1*parseInt(m[2]);
				} else if(ops.match(/\d+/)) {
		...
		if(fops !== undefined && fops.match(/d/)) {
			...
			var storagePenalty = -10;
			var islPenalty = -10;
			if(fops.match(/(\d+,(?:a|\d+),\d+)/)) {
				var m = fops.match(/(\d+,(?:a|\d+),\d+)/)[1].split(',');
				planningHorizon = parseInt(m[0]);
				if(m[1]==='a') {
					storagePenalty = null;
				} else {
			...
			var storagePenalty = -10;
			var islPenalty = -10;
			if(fops.match(/(\d+,\d+,(?:a|\d+),\d+)/)) {
				var m = fops.match(/(\d+,\d+,(?:a|\d+),\d+)/)[1].split(',');
			...
			var islPenalty = -10;
			if(/(\d+,\d+,\d+,(?:a|\d+),\d+)/) {
				var m = fops.match(/(\d+,\d+,\d+,(?:a|\d+),\d+)/)[1].split(',');
				downlinkCost = parseInt(m[0]);
				crosslinkCost = parseInt(m[1]);
