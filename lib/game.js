    ...
    Game.prototype.buildContext = function(seed, ops, fops) {
        ...
        for(var i = 0; i < this.numPlayers; i++) {
            ...
            if(ops !== undefined && ops.match(/s/)) {
				...
				var i = 6;
				if(ops.match(/\d+/)) {
					i = parseInt(ops.match(/(\d+)/)[1]);
				}
                operations = new fss.DynamicFederateOperations({planningHorizon:i});
            } else {
		...
		if(fops !== undefined && fops.match(/d+/)) {
			var i = 6;
			if(fops.match(/(\d+)/)) {
				i = parseInt(fops.match(/(\d+)/)[1]);
			}
			fedOperations = new fss.DynamicFederationOperations({planningHorizon:i});
		} else {
