...
define('game',function(require) {
    ...
    function Game() {
        ...
        this.eventTypes = [
            ...
            {number: /*5*/0, type: 'disturb', spec: {type: "Debris", hitChance: 1/6, maxHits: 1}}
        ];
    ...
    Game.prototype.buildContext = function(seed, ops, fops) {
        ...
        for(var i = 0; i < this.numPlayers; i++) {
            ...
            if(ops !== undefined && ops.match(/s/)) {
				...
				var storagePenalty = -10;
				var islPenalty = -10;
				if(ops.match(/(\d+,\d+,\d+)/)) {
					planningHorizon = parseInt(ops.match(/(\d+,\d+,\d+)/)[1].split(',')[0]);
					storagePenalty = -1*parseInt(ops.match(/(\d+,\d+,\d+)/)[1].split(',')[1]);
					islPenalty = -1*parseInt(ops.match(/(\d+,\d+,\d+)/)[1].split(',')[2]);
				} else if(ops.match(/\d+/)) {
					planningHorizon = parseInt(ops.match(/(\d+)/)[1]);
				}
                operations = new fss.DynamicFederateOperations({
		...
		if(fops !== undefined && fops.match(/d/)) {
			...
			var islPenalty = -10;
			if(fops.match(/(\d+,\d+,\d+)/)) {
				planningHorizon = parseInt(fops.match(/(\d+,\d+,\d+)/)[1].split(',')[0]);
				storagePenalty = -1*parseInt(fops.match(/(\d+,\d+,\d+)/)[1].split(',')[1]);
				islPenalty = -1*parseInt(fops.match(/(\d+,\d+,\d+)/)[1].split(',')[2]);
			} else if(fops.match(/(\d+)/)) {
				planningHorizon = parseInt(fops.match(/(\d+)/)[1]);
			} 
			fedOperations = new fss.DynamicFederationOperations({
				planningHorizon:planningHorizon,
				storagePenalty:storagePenalty,
				islPenalty:islPenalty
			});
		} else if(fops !== undefined && fops.match(/f/)) {
			var downlinkCost = 50;
			var crosslinkCost = 20;
			var storagePenalty = -10;
			var islPenalty = -10;
			if(fops.match(/(\d+,\d+,\d+,\d+)/)) {
				var m = fops.match(/(\d+,\d+,\d+,\d+)/)[1].split(',');
				downlinkCost = parseInt(m[0]);
				crosslinkCost = parseInt(m[1]);
				storagePenalty = -1*parseInt(m[2]);
				islPenalty = -1*parseInt(m[3]);
			} else if(fops.match(/(\d+,\d+)/)) {
				var m = fops.match(/(\d+,\d+,\d+)/)[1].split(',');
				downlinkCost = parseInt(m[0]);
				crosslinkCost = parseInt(m[1]);
			} 
			fedOperations = new fss.FixedCostFederationOperations({
				downlinkCost:downlinkCost,
				crosslinkCost:crosslinkCost,
				storagePenalty:storagePenalty,
				islPenalty:islPenalty
			});
		} else if(fops !== undefined && fops.match(/x/)) {
			var planningHorizon = 1;
			var downlinkCost = 50;
			var crosslinkCost = 20;
			var storagePenalty = -10;
			var islPenalty = -10;
			if(/(\d+,\d+,\d+,\d+,\d+)/) {
				var m = fops.match(/(\d+,\d+,\d+,\d+,\d+)/)[1].split(',');
				downlinkCost = parseInt(m[0]);
				crosslinkCost = parseInt(m[1]);
				planningHorizon = parseInt(m[2]);
				storagePenalty = -1*parseInt(m[3]);
				islPenalty = -1*parseInt(m[4]);
			} else if(fops.match(/(\d+,\d+,\d+)/)) {
				var m = fops.match(/(\d+,\d+,\d+)/)[1].split(',');
				downlinkCost = parseInt(m[0]);
				crosslinkCost = parseInt(m[1]);
				planningHorizon = parseInt(m[2]);
			} 
			fedOperations = new fss.DynamicFixedCostFederationOperations({
				downlinkCost:downlinkCost,
				crosslinkCost:crosslinkCost,
				planningHorizon:planningHorizon,
				storagePenalty:storagePenalty,
