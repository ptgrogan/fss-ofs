...
define(function(require) {
	...
	Subsystem.prototoype.getContentsSize = function() {
		var dataSize = 0;
		for(var i = 0; i < contents.length; i++) {
			dataSize += contents[i].size;
		}
		return dataSize;
	};
	...
	Subsystem.prototype.canTransferOut = function(data) {
		return this.nextContents.indexOf(data) > -1;
	};
	
	Subsystem.prototype.transferOut = function(data) {
		if(this.canTransfer(this.nextContents.indexOf(data)) {
			this.nextContents.splice(index, 1);
			return true;
		} else {
			return false;
		}
	...
	Subsystem.prototype.transmit = function(data) {
		if(this.canTransmit(data)) {
			this.nextContents.splice(this.nextContents.indexOf(data), 1);
	...
	Subsystem.prototype.init = function(sim) { 
		this.nextContents = this.contents;
		this.sensed = 0;
		this.transmitted = 0;
		this.received = 0;
	};
	...
	Subsystem.prototype.tock = function(sim) {
		this.contents = this.nextContents;
		this.sensed = 0;
		this.transmitted = 0;
		this.received = 0;
	};
