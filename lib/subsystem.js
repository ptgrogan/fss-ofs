...
define('subsystem', function(require) {
    ...
    function Subsystem() {
        ...
        this.maxTransferred = 2; // max bits transferred per turn
    ...
    /**
     * Checks if this subsystem can exchange data with another subsystem.
     * @param {object} data - The data.
     * @param {object} subsystem - The other subsystem.
     * @returns {Boolean} True, if contents can be exchanged.
     */
    Subsystem.prototype.canExchange = function(data, subsystem) {
        return this.transferred + data.size <= this.maxTransferred
				&& subsystem.transferred + data.size <= subsystem.maxTransferred
				&& (subsystem.isSensor()?subsystem.phenomena===data.phenomena:true)
				&& this.canTransferOut(data)
				&& _.some(subsystem.contents, function(otherData) {
					return subsystem.canTransferOut(otherData) 
							&& (this.isSensor()?this.phenomena===otherData.phenomena:true);
				}, this);
    };
     ...
     * @returns {Boolean} True, if contents can be exchanged.
     */
    Subsystem.prototype.exchange = function(data, subsystem) {
		if(this.canExchange(data, subsystem)) {
			var otherData = _.find(subsystem.contents, function(otherData) {
				return subsystem.canTransferOut(otherData) 
						&& (this.isSensor()?this.phenomena===otherData.phenomena:true);
			}, this);
			if(this.transferOut(data) 
					&& subsystem.transferOut(otherData)
					&& this.transferIn(otherData)
					&& subsystem.transferIn(data)) {
				return true;
			}
		}
        return false;
