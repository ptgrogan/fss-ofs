...
define('dynamicFixedCostFederationOperations', function(require) {
     ...
     */
	DynamicFixedCostFederationOperations.prototype.getStoragePenalty = function(demand, elapsedTime, spacecraft, context) {
		...
		return _.memoize(function(demand, elapsedTime, spacecraft) {
					return -1.0 / _.size(context.events) * 
						_.reduce(_.filter(context.events, function(event) {
							...
							return memo + Math.max(0, event.getValueAt(0) - demand.getValueAt(elapsedTime));
						}, 0, this);
				...
				_.each(ownSpacecrafts, function(spacecraft, i) {
					...
					_.each(demands, function(demand, j) {
						...
						if(this.storagePenalty !== null) {
							...
							J.Add(E_d[t][i][j], this.getStoragePenalty(
					...
					_.each(ownContracts, function(contract, j) {
						...
						if(this.storagePenalty !== null) {
							...
							J.Add(E_c[t][i][j], this.getStoragePenalty(
