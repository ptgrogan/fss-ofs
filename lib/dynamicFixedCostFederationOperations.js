...
define('dynamicFixedCostFederationOperations', function(require) {
    ...
    DynamicFixedCostFederationOperations.prototype.operate = function(federation, context) {		
		...
		var autoStoragePenalties = [];
		if(this.storagePenalty===null) {
			// auto-estimate storage penalties as expected 1-turn value of possible demands
			_.each(_.filter(phenomenas, function(phenomena) {
				return phenomena !== undefined;
			}), function(phenomena) {
				autoStoragePenalties[phenomena] = -1.0 / _.size(context.events) 
						* _.reduce(_.filter(context.events, function(event) {
							return event.isDemand() && event.phenomena === phenomena;
						}), function(memo1, demand) {
							return memo1 + demand.getValueAt(0);
						}, 0, this);
			}, this);
		}
		...
		_.each(federation.federates, function(federate) {
			...
			var L_c = []; // L_c[t][i][j][k][l]: at time t transmit data from spacecraft i to spacecraft j using protocol k for own contract l
			...
			_.each(ownSpacecrafts, function(spacecraft, i) {
				...
				_.each(demands, function(demand, j) {
					...
					C.push({
						...
						constraint: 'LE',
						constant: (spacecraft.couldSense(demand, context)?1:0),
			...
			_.each(_.range(minTime, maxTime, 1), function(time, t) {
				...
				_.each(ownSpacecrafts, function(spacecraft, i) {
					...
					_.each(demands, function(demand, j) {
						...
						if(this.storagePenalty !== null) {
							...
							J.Add(E_d[t][i][j], autoStoragePenalties[demand.phenomena]);
						}
					}, this);
					_.each(ownContracts, function(contract, j) {
						...
						if(this.storagePenalty !== null) {
							...
							J.Add(E_c[t][i][j], autoStoragePenalties[contract.demand.phenomena]);
						}
					}, this);
					_.each(phenomenas, function(phenomena) {
						var c = new lpsolve.Row();
						...
						_.each(SGLprotocols, function(protocol, k) {
							...
							_.each(demands, function(demand, l) {
								...
								C.push({
									...
									name: spacecraft.id + '-' + station.id + ' visibility' + ' at ' + time
								});
							}, this);
						...
						_.each(ISLprotocols, function(protocol, k) {
							...
							_.each(demands, function(demand, l) {
								...
								C.push({
									...
									name: spacecraft.id + '-' + spacecraft2.id + ' visibility' + ' at ' + time
								});
							}, this);
							_.each(ownContracts, function(contract, l) {
				...
				_.each(allSpacecrafts, function(spacecraft, i) {
					...
					_.each(demands, function(demand, j) {
						...
						if(time+1>=context.maxTime && SE_i > -1) {
							...
							C.push({
								row: new lpsolve.Row().Add(E_d[t][SE_i][j],1), 
								constraint: 'EQ', 
								constant: 0, 
					...
					_.each(ownContracts, function(contract, j) {
						...
						if(time+1>=context.maxTime && SE_i > -1) {
							...
							// (must also assign higher storage cost)
							C.push({
								row: new lpsolve.Row().Add(E_c[t][SE_i][j],1), 
								constraint: 'EQ', 
								constant: 0, 
