...
define('federate', function(require) {
    ...
    Federate.prototype.decommission = function(system, context) {
        ...
        if(index > -1) {
            //TODO this.cash += system.getDecommissionValue(context);
    ...
    Federate.prototype.contract = function(demand, context) {
        ...
        if(index > -1) {
            ...
            logger.info(this.id + " contracted for " + contract.demand.id);
            return contract;
        }
    };
    
    /**
     * Checks if this federate can sense data for a demand using a spacecraft.
     * @param {object} demand - The demand.
     * @param {object} spacecraft - The spacecraft.
     * @param {object} context - The context.
     * @returns {Boolean} True, if this federate can sense for the contract.
     */
    Federate.prototype.canSense = function(demand, spacecraft, context) {
        if(this.systems.indexOf(spacecraft) < 0) {
            logger.error("Federate does not control spacecraft.");
            ...
            return spacecraft.canSense(demand, context);
        }
        return false;
    }
    
    /**
     * Senses data for a contract using a spacecraft.
     * @param {object} contract - The contract.
     * @param {object} spacecraft - The spacecraft.
     * @param {object} context - The context.
     * @returns {Boolean} True, if this federate sensed data for the contract.
     */
    Federate.prototype.sense = function(contract, spacecraft, context) {
        if(this.contracts.indexOf(contract) < 0) {
            logger.error("Federate does not own contract for " + contract.demand.id);
        } else if(this.canSense(contract.demand, spacecraft, context)
                && spacecraft.sense(contract, context)) {
            logger.info(this.id + " sensed data for " + contract.demand.id 
                    + " contract using " + spacecraft.getTag());
            return true;
        } else {
            logger.error("Could not sense data for " + contract.demand.id 
                    + " using " + spacecraft.getTag());
        }
        return false;
    };
    ...
    Federate.prototype.defaultContract = function(contract, context) {
        ...
        if(index > -1) {
			...
			context.pastEvents.push(contract.demand);
            logger.info(this.id + " defaulted on " + contract.demand.id + " contract for " + contract.demand.defaultValue);
            if(this.cash < 0) {
    ...
    Federate.prototype.deleteData = function(contract, context) {
        _.each(this.subsystems, function(subsystem) {
            subsystem.contents = _.reject(subsystem.contents, function(data) {
    ...
    Federate.prototype.resolveContract = function(contract, context) {
        ...
        if(index > -1 && contract.isCompleted(context)) {
            ...
            this.contracts.splice(index, 1);
			context.pastEvents.push(contract.demand);
            logger.info(this.id + " completed contract " + contract.demand.id + " for " + contract.getValue());
            return true;
        }
        return false;
    };
