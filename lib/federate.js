...
define(function(require) {
	...
	Federate.prototype.design = function(system) {
		if(system.getContentsSize() > system.maxSize) {
			...
			console.log(this.id + " designed system " + system.id + " for " + system.getDesignCost());
			return true;
		}
		return false;
	};
	...
	Federate.prototype.commission = function(system, location, context) {
		if(system.getCommissionCost(location) > this.cash) {
			...
			console.log(this.id + " commissioned system " + system.id + " for " + system.getCommissionCost(location));
			return true;
		}
		return false;
	};
	...
	Federate.prototype.decommission = function(system, context) {
		...
		if(index > -1) {
			...
			console.log(this.id + " decommissioned system " + system.id + " for " + system.getDecommissionValue(context));
			return true;
		} else {
			return false;
		}
	...
	Federate.prototype.sense = function(contract, system, context) {
		if(this.contracts.indexOf(contract) < 0) {
			...
			console.log(this.id + " sensed data for contract " + contract.id + " using system " + system.id);
		}
		return false;
	};
	...
	Federate.prototype.defaultContract = function(contract, context) {
		...
		if(index > -1) {
			...
			console.log(this.id + " defaulted contract " + contract.id + " for " + contract.demand.defaultValue);
			if(this.cash < 0) {
