...
define('system', function(require) {
    ...
    System.prototype.couldStore = function(phenomena, dataSize) {
        return this.getMaxCapacity(phenomena) >= dataSize;
    }
	...
	System.prototype.store = function(data) {
		if(this.canStore(data)) {
			...
			// prioritize storing data in non-sensor
			if(dataContainer.isStorage() && !dataContainer.isSensor()) {
                logger.verbose(this.getTag() + " already stores data in storage");
                return true;
			} else if(_.some(_.filter(this.subsystems, function(subsystem) {
				return subsystem.isStorage() && !subsystem.isSensor();
			}), function(storage) {
				return this.canTransfer(dataContainer, data, storage)
						&& this.transfer(dataContainer, data, storage);
			}, this)) {
                logger.verbose(this.getTag() + " stored data in storage");
                return true;
            } else if(dataContainer.isStorage() && dataContainer.isSensor()) {
                logger.verbose(this.getTag() + " already stores data in sensor");
                return true;
			} else if(_.some(_.filter(this.subsystems, function(subsystem) {
				return subsystem.isStorage() && subsystem.isSensor();
			}), function(storage) {
                ...
                logger.verbose(this.getTag() + " stored data in sensor");
                return true;
            }
		}
        return false;
