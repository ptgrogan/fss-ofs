...
define('system', function(require) {
     ...
     */
    System.prototype.canStore = function(phenomena, dataSize) {
    ...
    System.prototype.getUsedCapacity = function(phenomena) {
        return _.reduce(_.filter(this.subsystems, function(subsystem) {
            return (subsystem.isSensor() && subsystem.phenomena===phenomena) 
                    || (!subsystem.isSensor() && subsystem.isStorage());
     ...
     * @param {String} phenomena - The transmission protocol.
    ...
    System.prototype.couldTransmit = function(protocol, origLoc, destLoc, dataSize, destination, context) {
        if(!this.isCommissioned()) {
            ...
            return _.reduce(_.filter(this.subsystems, function(subsystem) {
                ...
                return memo || _.reduce(_.filter(destination.subsystems, function(subsystem) {
                    return subsystem.isTransceiver();
                }), function(memo2, receiver) {
                    return memo2 || transmitter.canTransmit(origLoc, dataSize, destLoc, receiver, context);
                }, false, this);
            }, false, this);
        }
        return false;
        ...
        if(!this.isCommissioned()) {
            ...
            return _.reduce(_.filter(this.subsystems, function(subsystem) {
                ...
                return memo || _.reduce(_.filter(destination.subsystems, function(subsystem) {
                    ...
                    return memo2 || (this.canTransfer(dataContainer, data, transmitter) 
        ...
        if(this.canTransmit(data, destination, context)) {
            ...
            return _.reduce(_.filter(this.subsystems, function(subsystem) {
                ...
                return memo || _.reduce(_.filter(destination.subsystems, function(subsystem) {
                    ...
                    return memo || (this.canTransfer(dataContainer, data, transmitter) 
                            && transmitter.canTransmit(origLoc, data.size, destLoc, receiver, context)
    ...
    System.prototype.couldReceive = function(protocol, origLoc, destLoc, origin, dataSize, context) {
        if(!this.isCommissioned()) {
            ...
            return _.reduce(_.filter(this.subsystems, function(subsystem) {
                ...
                return memo || _.reduce(_.filter(origin.subsystems, function(subsystem) {
                    return subsystem.isTransceiver();
                }), function(memo2, transmitter) {
                    return memo2 || receiver.canReceive(origLoc, transmitter, dataSize, destLoc, context);
                }, false, this);
            }, false, this);
        }
        return false;
    ...
    System.prototype.canReceive = function(origin, data, context) {
        ...
        // TODO verify available storage capacity for ISL
    };
        ...
        if(this.canReceive(origin, data, context)) {
            return _.reduce(_.filter(this.subsystems, function(subsystem) {
                ...
                return memo || _.reduce(_.filter(origin.subsystems, function(subsystem) {
                    ...
                    return memo2 || receiver.receive(origLoc, transmitter, data, destLoc, context);
                    // TODO verify available storage capacity for ISL
                }, false, this);
            }, false, this);
        }
    ...
    System.prototype.transfer = function(origin, data, destination) {
        if(this.canTransfer(origin, data, destination)) {
            return (origin===destination) || (origin.transferOut(data) 
